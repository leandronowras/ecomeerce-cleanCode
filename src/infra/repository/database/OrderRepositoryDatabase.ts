...
export default class OrderRepositoryDatabase implements OrderRepository{
    ...
    async save(order: Order): Promise<void> {
        const [orderData] = await this.connection.query("insert into ccca.order (code, cpf, issue_date, freight, sequence, coupon, total) values ($1, $2, $3, $4, $5, $6, $7) returning *", [order.getCode(), order.cpf.value, order.date, order.getFreight(), order.sequence, order.coupon?.code, order.getTotal()])
        for (const orderItem of order.getOrderItems()) {
            await this.connection.query("insert into ccca.order_item (id_item, id_order, price, quantity) values ($1, $2, $3, $4)", [orderItem.idItem, orderData.id_item, orderItem.price, orderItem.quantity])
        }
    }
    ...
    // ta sendo implementado so pra passar em um teste ( jamais chamar em producao )
